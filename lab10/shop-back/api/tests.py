...
class ApiEndpointTests(TestCase):
    ...
    def test_product_detail_returns_requested_product(self):
        response = self.client.get(reverse('product-detail', kwargs={'product_id': 8}))
    ...
    def test_category_detail_returns_requested_category(self):
        response = self.client.get(reverse('category-detail', kwargs={'category_id': 3}))

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {'id': 3, 'name': 'Keyboards'})

    def test_category_products_filters_by_category(self):
        response = self.client.get(reverse('category-products', kwargs={'category_id': 4}))
    ...
    def test_product_detail_returns_404_for_missing_product(self):
        response = self.client.get(reverse('product-detail', kwargs={'product_id': 999}))

        self.assertEqual(response.status_code, 404)
        self.assertIn('detail', response.json())

    def test_category_products_returns_404_for_missing_category(self):
        response = self.client.get(reverse('category-products', kwargs={'category_id': 999}))
